    ...
    <section className="bg-gray-50 py-12">
      <div className="max-w-7xl mx-auto px-4">
        <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8">
          ...
          <Link href="/jobs">
            <p className="mt-2 sm:mt-0 text-teal-600 hover:underline">
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {displayedCategories.map((cat) => (
            <div
              key={cat.id}
              className="bg-white rounded-xl shadow-md hover:shadow-lg transition-shadow p-6 flex flex-col justify-between border-l-4 border-teal-500"
            >
              ...
              <div className="flex justify-between items-center mt-auto pt-2 border-t">
                <div className="flex items-center text-gray-700">
                  <svg
                    className="w-5 h-5 fill-current text-amber-500 mr-1"
                    viewBox="0 0 20 20"
                  >
                ...
                <Link href={`/talent?category=${cat.id}`}>
                  <span className="text-teal-600 text-sm hover:underline">View talents →</span>
                </Link>
              </div>
            </div>
          ))}
        ...
        {categoriesWithMatrix.length > initialDisplayCount && (
          <div className="flex justify-center mt-8">
            <button
              ...
              className="bg-white text-teal-700 px-6 py-2 rounded-full font-medium hover:bg-teal-50 transition-colors border border-teal-200"
            >
